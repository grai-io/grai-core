...
from grai_source_cube.models import Collection, Column, Edge, NodeTypes, Question, Table
from grai_source_cube.package_definitions import config

T = TypeVar("T")

"""
These functions use a concept of multiple dispatch. If you're unfamiliar, the invoked function depends on the type
signature of it's arguments. e.g.
...
add(1, 2) -> 3
add("hello", "moto") -> "hello-moto"

Functionally, the CubeConnector produces nodes and edges of a type defined in models.py but we need to convert those
objects to SourcedNodes and SourcedEdges as defined in grai-schemas. This is the location where you decide
what metadata you want to track for each object and provide the mapping between the input model objects and what gets
saved in Grai.
"""
