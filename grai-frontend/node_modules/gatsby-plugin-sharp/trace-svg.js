...
exports.notMemoizedtraceSVG = async ({
  ...
  const options = healOptions(getPluginOptions(), { // use maxWidth/maxHeight as width/height if available
  ...
  const tmpFilePath = path.join(tmpDir, filenamify(`${file.internal.contentDigest}-${file.name}-${optionsHash}.${file.extension}`));
