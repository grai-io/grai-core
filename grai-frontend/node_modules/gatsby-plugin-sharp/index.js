...
} = require(`./gatsby-worker`);

const {
  getDimensionsAndAspectRatio
} = require(`./utils`);
...
const getImageSizeAsync = async file => {
  ...
  if (!dimensions) {
    reportError(`gatsby-plugin-sharp couldn't determine dimensions for file:\n${file.absolutePath}\nThis file is unusable and is most likely corrupt.`, ``);
  }
...
function prepareQueue({
  ...
  const {
    ...
    height,
    aspectRatio
  } = calculateImageDimensionsAndAspectRatio(file, options); // encode the file name for URL
  ...
  const prefixedSrc = (pathPrefix ? pathPrefix : ``) + `/static/${digestDirPrefix}` + encodedImgSrc;
  return {
    src: prefixedSrc,
    outputDir: outputDir,
...
function createJob(job, {
  ...
  if (!actions) {
    reporter.panic(`Gatsby-plugin-sharp wasn't setup correctly in gatsby-config.js. Make sure you add it to the plugins array.`);
...
function lazyJobsEnabled() {
  return process.env.gatsby_executing_command === `develop` && !isCI() && !(process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `true` || process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `1`);
}
...
function batchQueueImageResizing({
  ...
  const operations = [];
  const images = []; // loop through all transforms to set correct variables
  ...
  const finishedPromise = createJob({
    name: IMAGE_PROCESSING_JOB_NAME,
    inputPaths: [file.absolutePath],
    outputDir: path.join(process.cwd(), `public`, `static`, file.internal.contentDigest),
    args: {
      isLazy: lazyJobsEnabled(),
      operations,
      pluginOptions: getPluginOptions()
...
async function fluid({
  ...
  const maxWidth = options.maxWidth ? Math.min(options.maxWidth, width) : undefined;
  const maxHeight = options.maxHeight ? Math.min(options.maxHeight, height) : undefined;
  ...
  let filteredSizes = fluidSizes.filter(size => size < (fixedDimension === `maxWidth` ? width : height)); // Add the original image to ensure the largest image possible
  ...
  const imageWithDensityOneIndex = filteredSizes.findIndex(size => size === (fixedDimension === `maxWidth` ? maxWidth : maxHeight)); // Sort sizes for prettiness.
  ...
  const transforms = filteredSizes.map(size => {
    ...
    arrrgs[dimensionAttr] = Math.round(size); // we need pathPrefix to calculate the correct outputPath
  ...
  const presentationHeight = imageWithDensityOne.height; // If the users didn't set default sizes, we'll make one.
...
async function fixed({
  ...
  const transforms = _.sortBy(filteredSizes).map(size => {
    const arrrgs = createTransformObject(options);
    arrrgs[fixedDimension] = Math.round(size); // Queue images for processing.
