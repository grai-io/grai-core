...
async function prepareStackTrace(error, sourceOfMainMap) {
  ...
  const maps = await Promise.all(bundleDirMapFiles.map(async source => await new _sourceMap.SourceMapConsumer((0, _fs.readFileSync)(source, `utf8`))));
...
function getPosition({
  ...
  for (const map of maps) {
    const test = map.originalPositionFor({
      line,
      column
    });
