...
async function initialize({
  ...
  const workerCacheDirExists = _fsExtra.default.existsSync(workerCacheDirectory);

  const lmdbCacheDirExists = _fsExtra.default.existsSync(lmdbCacheDirectory); // check the cache file that is used by the current configuration
  ...
  try {
    ...
    if (lmdbStoreIsUsed) {
      await _fsExtra.default.ensureDirSync(`${cacheDirectory}/${lmdbCacheDirectoryName}`);
    } else {
      await _fsExtra.default.ensureDirSync(`${cacheDirectory}/json`);
