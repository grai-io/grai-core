...
<Callout type="info" emoji="ℹ️">
  This guide is written based on using [Grai cloud](https://app.grai.io), however, it will run equally well on your own self-hosted implementation.
</Callout>
...
<Image
    src="/images/docs/core/examples/snowflake-connection-button.png"
    height={200}
    width={200}
/>

Fill out the the connection details for your Snowflake data warehouse.
...
However you could set your connection to run every day insuring your lineage is up to date.
...
<Image
    src="/images/docs/core/examples/snowflake-connection2.png"
    height={400}
    width={800}
/>
...
Now that the connection has completed running you can go to the `Graph` page to see your snowflake tables.
It will likely looking something like this

<br/>

<Image
    src="/images/docs/core/examples/snowflake-graph.png"
    height={400}
    width={800}
/>
...
For the purposes of this demo, we've created a [manifest.json](https://raw.githubusercontent.com/grai-io/jaffle_shop_snowflake_demo/master/files/manifest.json) file you can use which
was created by running `dbt compile` on the source [repository](https://github.com/grai-io/jaffle_shop_snowflake_demo).
...
Press Finish to upload the file and run the sync.
This should take a few seconds before completing successfully.

Finally, check out the graph view again.
You should now see that your Snowflake tables have been joined together based from the dbt metadata.
...
This time, choose the postgres connection.
...
You're probably getting the hang of this now, but just in case, press continue.
Assuming your credentials are valid, you can press finish to complete the setup and then refresh to run the connection.
...
There won't be any links between the Postgres tables and the Snowflake ones.
...
Again, Leave Namespace as default. Upload the [edges.yaml]((https://github.com/grai-io/jaffle_shop_snowflake_demo/blob/master/files/edges.yaml)) file from the source [repository](https://github.com/grai-io/jaffle_shop_snowflake_demo).
...
<Callout type="info" emoji="ℹ️">
  You will need a Github repository for this stage.
  Forking the example repository might be a good starting point [jaffle_shop_snowflake_demo](https://github.com/grai-io/jaffle_shop_snowflake_demo).
</Callout>
...
<Callout type="info" emoji="ℹ️">
  Normally you would use the credentials for the development database that changes with each pull request or commit in order to evaluate your tests.
  In this case, we've created a sample database for you to use.
</Callout>
