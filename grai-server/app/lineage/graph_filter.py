...
def filter_by_filter(filter, query: GraphQuery) -> GraphQuery:
    for row in filter.metadata:
        filter_by_dict(row, query)

    return query


def filter_by_dict(row, query: GraphQuery) -> GraphQuery:
    value = row["value"]
    ...
    if row["type"] == "table":
        ...
        if row["field"] == "tag":
            if row["operator"] == "contains":
                query.optional_match("(table)<-[:TABLE_TO_TABLE|:TABLE_TO_TABLE_COPY*]-(othertable:Table)").withWhere(
